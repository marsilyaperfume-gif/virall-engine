...
v19.2 Stable Login Restore:
- Restored working Facebook Login OAuth scopes.
- Removed publish permission from OAuth so login does not break.
- Publish endpoint now returns a clear Meta permission message instead of breaking OAuth.
- Do not add instagram_content_publish to OAuth URL again.


v21 Publish Scope Enabled:
- Added instagram_content_publish to Facebook Login OAuth scopes.
- Kept existing working App ID / Secret flow unchanged.
- No changes to Cloudinary, UI, persistence, or publish-reel logic.
- After deploying, delete connected accounts and reconnect Instagram to generate a new token with publish permission.
